...
type
  ...
  protected
    ...
    FWettk            : TWettkObj;
    FKlasse           : TAkObj;
    FSerSumSort       : Int64; // Punkt- oder ZeitSumme in 1/100 Sek (sonst Integer-Überlauf)
    FSerSumLst        : Int64; // Punkt- oder ZeitSumme in 1/100 Sek (sonst Integer-Überlauf)
    FSerieRang        : Integer;
    ...
    FSerSumColl       : TWordCollection;    // Summe pro Ort für Serienwertung in Klasse
    FRndColl          : TWordCollection;    // Runden für waRndRennen (wkAbs1)
    FStreckenColl     : TIntegerCollection; // für Stundenrennen (wkAbs1)
  ...
  var MschGrListe : TIntegerCollection; // Größe für jedem MannschName in MannschNameColl
                                        // unter Berücksichtigung von SortKlasse und SortStatus
implementation
...
// MannschColl ist abgeleitet von TlnColl, deshalb hier nicht berücksichtigen
var P : PString;
begin
  P := GetNamePtr(Name);
  if P <> nil then
...
begin
  ...
  begin
    ...
    if Result > 0 then // gültige Endzeit  ??????????????????????? warum?, testen !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
begin
  ...
  Msch1      := nil; // beim Einlesen gesetzt, für WordUrk in anMschAStart und MschWertg = mwMulti
  FWettk     := nil;
  FKlasse    := AkUnbekannt;
...
//==============================================================================
// Punkte oder Zeitsumme für Serie berechnen, immer über alle Orte
// nicht gestartete Orte werden nicht berücksichtigt

var i, MschOrteGewertet, WkLetztOrtGewertet,
    ErstPktZahl,LetztPktZahl              : Integer; // auch für Zeiten bei Zeitaddition
...
begin
  ...
  with FWettk do
  begin
    // Streichzahl wird um Zahl der nicht durchgeführte Wettk gekürzt
...
begin
  ...
  begin
    ...
    for i:=0 to TVeranstObj(FVPtr).OrtZahl-1 do
      ...
      if MschOrtSerWertung(i) then // PktWrtg oder (ZeitWrtg für Ser und Msch)
        if TlnImZielColl[tmMsch][i] then // Wettk durchgeführt
        begin
          WkLetztOrtGewertet := i;
          if GetOrtRng(i) > 0 then // Disq nicht berücksichtigen
          begin
        ...
        end else // Wettk (noch) nicht durchgeführt
        begin
          SetPflichtWkErfuellt(i);
    ...
    begin
      ...
      if PflichtWkErfuellt and
        ...
        FSerieEndWertung := true; // Endwertung erreichbar oder erreicht, für Sortierung
    end;
    ...
    for i:=0 to SerPktBuffColl.Count-1 do // Count = TVeranstObj(FVPtr).OrtZahl
      ...
      begin
        ...
        begin
          FSerSumSort := FSerSumSort + SerPktBuffColl[i]; // Int64, sonst Überlauf
...
// FWettk.MschModified nicht setzen und MannschName nicht löschen
        ...
        if (Wettk=WettkAlleDummy) or (Wettk=GetPItem(i).FWettk) then
          for k:=0 to ReportAkListe.Count-1 do
            ...
            begin
              ...
              Break; // nur eine Klasse ist gültig pro Mannschaft
            end;
...
begin
  ...
  // nur für waMschStaffel neu definieren
  if WertungsWk.WettkArt = waMschStaffel then
    for i:=0 to Count-1 do with GetPItem(i) do
    begin
      if (FWettk=WertungsWk) and (FKlasse=AkAlle) then
      begin
        // Tln nach EndUhrzeit rel. zur SGrp.Startzeit/EinzelStartzeit sortieren,wegen Tagesübergang
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SetDisq(false);
        Runden   := 0;
        Strecken := 0; // Msch-Wettkämpfe nicht als Stundenrennen
...
//==============================================================================
// Wertung für waEinzel,waRndRennen,waStndRennen, keine Wertung bei waTlnStaffel oder waTlnTeam
...
begin
  if WertungsWk.MschWettk then
    MannschWkWertung(WertungsWk) // waMschStaffel,waMschTeam nur für AkAlle
...
//==============================================================================
// für Anzeige StatusBar
// Msch nur zählen wenn genügend Tln gemeldet
...
begin
  ...
  if EinlWettk.MschWertg = mwMulti then
    ...
    begin
      ...
      AddItem(Mschj);
      if j=1 then Msch0.Msch1 := Mschj; // Pointer für WordUrk
      for k:=0 to Msch0.FTlnListe.Count-1 do
        Mschj.FTlnListe.AddItem(Msch0.FTlnListe[k]); // geändert 11.5.5
    end;
end;
...
// damit Zuordnung über Tagesgrenze hinaus gültig
      ...
      if Wettk=Wk then
        if (DummyTln.SGrp<>nil)and(DummyTln.Snr>0)and(GetAbsStZeit(Abs_1)>=0) then
          case DummyTln.SGrp.StartModus[Abs] of
            stOhnePause:
              SetAbsStZeit(Abs,GetAbsStZeit(Abs_1)+GetAbsZeit(Abs_1)); // tagesüberlauf wird korrigiert
            stMassenStart:
...
// StZeit vorher berechnet, für Team
begin
...
// StZeit vorher berechnet, für Staffel
var AbsCnt : TWkAbschnitt;
    Zeit   : Integer;
begin
  Result := 0;
...
begin
  ...
  case Wk.WettkArt of
    ...
    begin
      ...
      // stAbs1Ziel,stImZiel setzen gültige Tln-AbsZeit voraus und damit gültige AbsStZeit,
      // diese wird aber erst später, nach MschZeiten gesetzt
      AbsStatus := TStatus(Integer(stAbs1UhrZeit)+Integer(Abs)-1);
      // nur Tln mit gültiger Stoppzeit für den Abschnitt und nicht disq.
      for i:=0 to Count-1 do with GetPItem(i) do
        if FWettk=Wk then
        begin
          ...
          if StZeit >= 0 then
          begin
            // alle Tln nach Abs-UhrZeit rel. zur Abs1-Startzeit sortiert, nur Tln mit gültigen Zeiten
            // Tagesübergang berücksichtigt
            FTlnliste.SortMode := smNichtSortiert;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              // stImZiel NICHT benutzen, weil StZeiten noch nicht gültig
...
//==============================================================================
// Tln in Mannschaft löschen, wenn Mannsch vorhanden
// im TlnDialog benutzt vor TlnEntfernen und vor Änderung (TlnAktuell <> nil)
...
begin
  ...
  begin
    ...
    begin
      // Tln in weiteren Msch löschen; wenn Index=0 andere Msch, also abbrechen
...
//==============================================================================
// Reihenfolge: Alle, Männer, Frauen, Mixed, Ak-männl., Ak-weibl.
begin
