  ...
  begin
    ...
    try
      SortSMld.Init(Trim(SMldNameEdit.Text),Trim(SMldVNameEdit.Text),
                    ...
                    SMldPLZEdit.Text,SMldOrtEdit.Text,GetMannschName,
  ...
  begin
    ...
    try
      ...
      begin
        ...
        for i:=0 to SortCount-1 do SMldVereinCB.Items.Append(SortItems[i]^);
      end;
      if (SortSMld.MannschNamePtr<>nil) and
         (Veranstaltung.MannschNameColl.SortCount>0) then
        SMldVereinCB.ItemIndex :=
              Veranstaltung.MannschNameColl.SortIndexOf(SortSMld.MannschNamePtr)+1
  ...
  with HauptFenster do
    if SortSMld = nil then Result := false
      ...
      Result := not StrGleich(SMldNameEdit.Text,SortSMld.Name) or
                ...
                not StrGleich(GetMannschName,SortSMld.MannschName);
end;
...
begin
  with HauptFenster do
    ...
    if NameSortRB.Checked and (FSMldSortMode<>smSMldName) or
       VereinSortRB.Checked and (FSMldSortMode<>smSMldMschName) then
    begin
      if VereinSortRB.Checked then FSMldSortMode := smSMldMschName
