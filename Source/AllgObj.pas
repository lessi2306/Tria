...
type
  ...
  protected
    //FLimit    : SmallInt; // um DOS-Kompatibel spechern zu können
    //FDelta    : SmallInt; // um DOS-Kompatibel spechern zu können
    FItems    : TList;
    FItemSize : Integer; // ProgressBar beim Speichern/Löschen, Wert nicht exact
    ...
    procedure FreeItem(Item:Pointer); virtual; abstract; // Item-Inhalt löschen
...
{    function    Compare(Item1,Item2:Pointer):Integer; override;
  ...
  // wie Integer, SmallInt nur für Tri-Datei
  protected
    function  GetBPObjType: Word; override;
    function  LoadItem(Indx:Integer): Boolean; override;
    function  StoreItem(Indx:Integer): Boolean; override;
  ...
  // wie SmallInt, nur für Tri-Datei wegen Compatibilität mit DOS-Version
  protected
    function  GetBPObjType: Word; override;
    function  LoadItem(Indx:Integer): Boolean; override;
    function  StoreItem(Indx:Integer): Boolean; override;
  ...
  // wird nicht gespeichert, nur für serienwertung benutzt (SerPktBuffColl)
  protected
    function    GetBPObjType: Word; override;
    function    LoadItem(Indx:Integer): Boolean; override;
    function    StoreItem(Indx:Integer): Boolean; override;
...
{$IFDEF TRIAPROJEKT} // nicht für TriaZeit
...
begin
  ...
  begin
    S := '';
    if (Read(S[0],1)=1) and  // Längen-Byte
...
begin
  S := AnsiString(Str);
  L := Length(S);
  //Size := L * SizeOf(Char); // Puffergröße in Byte angeben, UniString
  if (Write(L, SizeOf(LongInt)) <> SizeOf(LongInt)) or // Länge in 4 Byte
...
//------------------------------------------------------------------------------
begin
  // 2003-1,7: nil nicht sinnvoll, löst weitere exceptions aus
...
begin
  ...
  begin
    ...
    FreeItem(GetPItem(Indx)); //2003-1.7: nil wird in FreeItem überprüft
    // Pointer(Item) aus Liste (FItems) entfernen, auch wenn Pointer=nil
    FItems.Delete(Indx);
    //muss in Nachfolger überschrieben werden
...
//==============================================================================
// verwendet für ProgressBar während Schliessen, Speichern?
begin
...
//------------------------------------------------------------------------------
begin
  // keine Aktion, Pointer-Inhalt wird nicht gelöscht
end;
...
//==============================================================================
// Liste muß nach aktueller SortString sortiert sein, sonst falsches Ergebnis
...
//==============================================================================
begin
  // Unterschied Groß/Klein-Buchstaben berücksichtigen
  // bei ANSICompareStr Kleinbuchstaben kleiner als Großbuchstaben,
...
begin
  ...
  if (FSortItems<>nil) and (FSortItems.Capacity < FItems.Capacity) then
    FSortItems.Capacity := FItems.Capacity; // unabhängig von TList.Add
  AddSortItem(Item);
end;
...
//------------------------------------------------------------------------------
begin
  {2003-1.7: Prüfung streichen um zu beschleunigen
...
begin
  ...
  (* AddItem bei TOrtCollection überschrieben, InsertItem nicht *)
end;
...
//==============================================================================
// wie TriaColl, aber mit ProgressBar, nicht für TriaZeit
...
begin
  if S <> '' then
  begin
    New(P);  // EOutOfMemory-Exception, wenn nicht genügend Speicherplatz
    P^:= S;
    Result := AddItem(P);
...
// unabhängig von ReadStr, weil Version dort benutzt wird
var S4 : String[4];
    S  : ShortString;
begin
  Result := false;
  try
    ...
    S := '';
    TriaStream.ReadBuffer(S[0],1); // Längen-Byte
    if (Ord(S[0]) > 0) then
...
begin
  ...
  try
    ...
    TriaStream.WriteBuffer(S,Ord(S[0])+1); //benutzte Länge statt 256
  except
    Exit;
  end;
  Result := true;
