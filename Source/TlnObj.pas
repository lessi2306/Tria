...
type
  ...
  protected
    ...
    FSerSumCollArr    : TSerRngCollArr;  // Seriensumme pro Ort, für kwAlle,kwSex,kwTlnAk,kwSondKl
...
//------------------------------------------------------------------------------
// nur Inhalt von Pointer übernehmen
begin
...
//------------------------------------------------------------------------------
begin
  Result := PZeitRec(inherited GetPSortItem(Indx)); // Indx geprüft
end;
...
begin
  if GetPSortItem(Indx) <> nil then // Indx geprüft
...
begin
  ...
  try
    ...
    if Indx <= 1 then // StrtZeit und Runde=1 auch ungültige AufrundZeit
      SetzeZeitRec(Indx,I1,I2)
    else
    if I2 >= 0 then // nur gültige Rundenzeiten laden
      SetzeZeitRec(Count-1,I1,I2); // letztes (Leer)Item setzen, neues LeerItem zugefügt
    Result := true;
  except
    Exit;
  end;
...
//------------------------------------------------------------------------------
// FMaxRunden = cnRundenMax für Runden-Rennen
begin
  if (MaxNeu < 1) or (MaxNeu > cnRundenMax) then Exit;

  FMaxRunden := MaxNeu;
  // alle Leerzeiten löschen, damit überzählige Leerzeiten verschwinden
  while (SortCount > 0) and (PZeitrec(FSortItems.Last)^.AufrundZeit < 0) do
    ClearItem(FSortItems.Last);
  // überzählige Zeiten löschen
...
//==============================================================================
begin
  if Count < FMaxRunden + 1 then // zusätzlich Startzeit
...
begin
  ...
  if GetItem(0).AufrundZeit >= 0 then // Startzeit gültig
    if (Item<>nil) and (PZeitRec(Item)^.AufrundZeit >= 0) then // Rundenzeit gültig
    begin
...
//==============================================================================
// mit AufrundZeit = -1 Item löschen
...
begin
  ...
  begin
    ...
    if (AufrundZeitNeu <> PZeitRec(FItems[Indx])^.AufrundZeit) and
      ...
      begin
        if AufrundZeitNeu < 0 then ClearIndex(Indx) // ungültige Zeit löschen
        else
        begin
          ClearSortItem(FItems[Indx]);  // aktuelles item neu sortieren
          PZeitRec(FItems[Indx])^.AufrundZeit := AufrundZeitNeu; // nur gültige Zeit setzen
          Result := AddSortItem(FItems[Indx]);
        end;
        // leere Zeit am Ende, wenn MaxCount (+1 für Startzeit) noch nicht erreicht
...
//==============================================================================
// MaxCount, ErfZeiten nicht übernehme (ErfZeit := -1)
var i : Integer;
begin
  Clear;
...
begin
  ...
  // Groß/Klein-Unterschied wird überschrieben
...
begin
  ...
  begin
    ...
    if (FVPtr=Veranstaltung) and (FWettk<>nil) and (CollectionIndex >= 0) then
      FWettk.KlassenModified := true; // auch MannschModified, ErgModified für alle Orte
    KlassenSetzen; // für MschKlassen für Tln ausserhalb TlnColl
    SortAdd;
  end;
end;
...
//------------------------------------------------------------------------------
// für jeden Ort eigener Eintrag in DisqNameColl
    ...
    kwSex    : if TM = tmTln then
                 ...
                 Result := FMschWrtg and
                           ((FSex = Klasse.Sex) and not FMschMixWrtg or
                            (Klasse.Sex=cnMixed) and FMschMixWrtg); // unabhängig von FSex
  ...
  case Status of
    ...
    stEingeteilt:  // Snr>0 sollte reichen, SBhn > 0 wenn StrtBahnen definiert
      Result := (FSGrpColl[Indx] <> nil) and (FSnrColl[Indx] > 0) and
                ((FWettk.OrtStartBahnen[Indx]=0) or (FStrtBahnColl[Indx]>0)); // RfidCode nicht berücksichtigt
        ...
        for AbsCnt:= wkAbs1 to TWkAbschnitt(FWettk.OrtAbschnZahl[Indx]) do
          if (AbsRundeStoppZeit(Indx,AbsCnt,1) >= 0) or // StoppZeit eingelesen
             ...
             ((TSGrpObj(FSGrpColl[Indx]).StartModus[wkAbs1]=stOhnePause)and(StrtZeit(Indx,wkAbs1)>=0) or// Startzeit eingelesen
              (FWettk.OrtWettkArt[Indx]=waStndRennen)and(Gesamtstrecke(Indx)>0)) then // bei waStndRennen zählt zusätzlich die Strecke
          begin
            Result := true;
            Exit;
          end;
    ...
    stAbs1UhrZeit..stAbs8UhrZeit: // gültige StoppZeit ohne Disq, unabhängig von Startzeit,
...
//==============================================================================
// AufrundZeit übernommen,  ErfZeit := -1
var AbsCnt : TWkAbschnitt;
begin
  SortRemove;
  if (FVPtr=Veranstaltung) and (FWettk<>nil) and (CollectionIndex >= 0) then
...
//==============================================================================
// Abs > wkAbs0, keine überprüfung
// Startzeit für Abs > 0 setzen, nur AufrundZeit, ErfZeit für EinzelStart nicht ändern
...
// FLoadPtr benutzen für Berechnung im TlnDlg
            ...
            if (TTlnObj(FLoadPtr).CollectionIndex >= 0) and
              ...
              if SG.StartModus[Abs] <> stOhnePause then
                if Abs_1Zeit > 0 then
                  ...
                  StZeitNeu := -1 // bei not stOhnePause muss vorhergehender AbsZeit gültig sein
...
// überzähliche Rundenzeiten werden gelöscht
begin
...
// waRndRennen/waStndRennen: Endzeit mit unvollständigen Rundenzahlen in wkAbs1
...
begin
  ...
  begin
    ...
    if Mode = lmExport then
      Result := ExpZeitSekStr(Buf) // mit führende 00
    else
...
//==============================================================================
// Punkte/Zeitsumme für Serie berechnen, immer über alle Orte
...
begin
  ...
  begin
    ...
    //   wird bei Punkteberechnung nicht berücksichtigt)
    // Items nicht löschen, damit PflichtWkOrtIndx sich nicht ändert
    StreichergEntfernen;

    // SerieSumme über Orte summieren
      ...
      if SerPktBuffColl[i] <> ErstPktZahl then // Rng=0 incl. StreichErgebnisse ignorieren
        if SerPktBuffColl[i] = LetztPktZahl then // Rng war ursprünglich 0,
          ...
          if SerWrtgMode[tmTln] = swZeit then // Summe=cnTlnSerOrtZeitMax
            ...
            //SerieSumLst unverändert (Sum=0)
          else // Summe=Punkte für Rng=0
          begin
...
begin
  ...
  begin
    ...
    if FWettk.WettkArt = waStndRennen then // nur 1 Abschnitt, Gesamtstrecke
      if Gesamtstrecke > 0 then // stGewertet, unabhängig von Startzeit
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                // Zt unverändert, entspricht Startzeit 00:00:00
...
begin
  ...
  // Verein und MannschName in Coll Löschen wenn nicht mehr benutzt
...
begin
  ...
  begin
    Inc(WrtgArr[Integer(ixRngZahl),KlasseIndx,SexIndx]); // RangZahl für jeden Tln erhöht
    ...
    begin
      ...
      WrtgArr[Integer(ixRngBuff),KlasseIndx,SexIndx] :=
                          WrtgArr[Integer(ixRngZahl),KlasseIndx,SexIndx]; // RngZahl als Rng übernehmen
...
// auf Mannsch oder Verein prüfen: beide müssen unterschiedlich sein
var i : Integer;
    S1,S2 : String;
begin
  Result := nil;
...
begin
  ...
  if RfidHex then
    for i:=0 to FItems.Count-1 do
      if SameText(GetPItem(i).GetRfidCode,Code) then // Groß/Klein nicht unterscheiden
      begin
        Result := GetPItem(i);
        Exit;
      end else
    ...
    for i:=0 to FItems.Count-1 do
      if SameStr(GetPItem(i).GetRfidCode,Code) then // Groß/Klein unterscheiden
      begin
        Result := GetPItem(i);
        Exit;
      end;
...
// manuell eingetragene, geänderte oder importierte Zeiten nur in AufrundZeit
  ...
  begin
    ...
    if TlnPtr.FWettk = Wk then
      ...
      begin
        ...
        begin
          //TlnPtr.GetZeitRecColl(Ix,Abs).SetAufrundZeit(j,-1); // AufrundZeit löschen, nicht nötig
          ZEObj := TZEObj.Create(Veranstaltung,ErfZeitColl,oaAdd);
          with ZEObj do
          begin
            Tln   := TlnPtr;
            Runde := j; // Index für spätere Eintragung, nicht Runde
            EinleseZeit := ZeitNeu;
          end;
