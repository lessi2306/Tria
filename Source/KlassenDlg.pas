...
begin
  ...
  begin
    ...
    if WkAktuell.MschWettk then  // Wertung nur für Alle Tln
    begin
...
//------------------------------------------------------------------------------
// aktuelle Klassenliste nicht einbeziehen, wird in KlasseOk vorher geprüft
var Coll:TAkColl;
    S : String;
//------------------------------------------------------------------------------
  ...
  begin
    ...
    begin
      TriaMessage(Self,'Die Bezeichnung  "'+Trim(NameEdit.Text)+
                  '"  wird bereits für  "Alle Klassen"  verwendet.',
                   mtInformation,[mbOk]);
      Result := true;
      Exit;
    end;
    ...
    begin
      TriaMessage(Self,'Die Bezeichnung  "'+Trim(NameEdit.Text)+
                  '"  wird bereits für die  "'+S+'Männerklasse"  verwendet.',
                   mtInformation,[mbOk]);
      Result := true;
      Exit;
    end;
    ...
    begin
      TriaMessage(Self,'Die Bezeichnung  "'+Trim(NameEdit.Text)+
                  '"  wird bereits für die  "'+S+'Frauenklasse"  verwendet.',
                   mtInformation,[mbOk]);
      Result := true;
      Exit;
    end;
    // prüfe Ak-Männer-Name
    if (Coll <> AltMKlasseColl[TlnMschSelected]) and
       AkNameInColl(NameEdit.Text,AltMKlasseColl[TlnMschSelected]) then
    begin
      TriaMessage(Self,'Die Bezeichnung  "'+Trim(NameEdit.Text)+
                  '"  wird bereits für eine '+S+'Altersklasse - Männlich verwendet.',
                   mtInformation,[mbOk]);
      Result := true;
      Exit;
    end;
    // prüfe Ak-Frauen-Name
    if (Coll <> AltWKlasseColl[TlnMschSelected]) and
       AkNameInColl(NameEdit.Text,AltWKlasseColl[TlnMschSelected]) then
    begin
      TriaMessage(Self,'Die Bezeichnung  "'+Trim(NameEdit.Text)+
                  '"  wird bereits für eine '+S+'Altersklasse - Weiblich verwendet.',
                   mtInformation,[mbOk]);
      Result := true;
      Exit;
    end;
    // prüfe Ak-Männer-Kürzel
    if TlnMschSelected = tmTln then
    begin
      if (Coll <> AltMKlasseColl[tmTln]) and
         AkKuerzelInColl(KuerzelEdit.Text,AltMKlasseColl[tmTln]) then
      begin
        TriaMessage(Self,'Das Kürzel  "'+Trim(KuerzelEdit.Text)+
                    '"  wird bereits für eine Altersklasse - Männlich verwendet.',
                     mtInformation,[mbOk]);
        Result := true;
        Exit;
      end;
      //prüfe Ak-Frauen-Kürzel
      if (Coll <> AltWKlasseColl[tmTln]) and
         AkKuerzelInColl(KuerzelEdit.Text,AltWKlasseColl[tmTln]) then
      begin
        TriaMessage(Self,'Das Kürzel  "'+Trim(KuerzelEdit.Text)+
                    '"  wird bereits für eine Altersklasse - Weiblich verwendet.',
                     mtInformation,[mbOk]);
        Result := true;
        Exit;
      end;
      ...
      begin
        TriaMessage(Self,'Die Bezeichnung  "'+Trim(NameEdit.Text)+
                    '"  wird bereits für eine Sonderklasse - Männlich verwendet.',
                     mtInformation,[mbOk]);
        Result := true;
        Exit;
      end;
      ...
      begin
        TriaMessage(Self,'Die Bezeichnung  "'+Trim(NameEdit.Text)+
                    '"  wird bereits für eine Sonderklasse - Weiblich verwendet.',
                     mtInformation,[mbOk]);
        Result := true;
        Exit;
      end;
...
(*----------------------------------------------------------------------------*)
// bei Änderung zunächst nur Klasse prüfen, nicht die Konsistenz der Liste
var i : Integer;
begin
  Result := false;
  ...
  begin
    TriaMessage(Self,'Kürzel der Klasse fehlt.',mtInformation,[mbOk]);
  ...
  begin
    ...
    begin
      TriaMessage(Self,'"Alter von" ist größer als "Alter bis".',mtInformation,[mbOk]);
      if VonEdit.CanFocus then VonEdit.SetFocus;
      Exit;
    end;
  end;
...
begin
  ...
  if AkGeAendert then
    ...
    case TriaMessage(Self,'Die Einstellungen der markierten Klasse wurden geändert.'+#13+
...
begin
  if AkAktuell = nil then Exit;
  if not NeuEingabe and (TriaMessage(Self,'Markierte Klasse löschen?',
...
// Prüfung auf Doppel-Namen/-Kürzel vorher in KlasseOk
var i,AlterBuf : Integer;
begin
  Result := false;
  // leere Liste ist zulässig
  // Alters-Überlappung nicht zulässig
  AlterBuf := -1;
  for i:=0 to AkGrid.ItemCount-1 do
      ...
      if AlterVon > AlterBuf + 1 then
        ...
        begin
          TriaMessage(Self,'Das Alter  "'+IntToStr(AlterBuf+1)+
                      ...
                      'In der Klassenliste sind Alterslücken nicht erlaubt.',
                       mtInformation,[mbOk]);
          AkGrid.ItemIndex := i;
          SetAkDaten;
          if VonEdit.CanFocus then VonEdit.SetFocus;
...
begin
  ...
  if NeuEingabe then // bei NeuEingabe Zusätzlicher Ak in AkGrid[0]
...
begin
  ...
  try
    ...
    begin
      ...
      case SexSelected of
        //cnSexBeide  : AkAlle, AkMixed nicht änderbar
...
begin
  ...
  case DefListe of
    alDTU:
      case WertgSelected of // keinen Unterschied für Tln-Msch
        kwAltKl  : case SexSelected of
...
begin
  ...
  try
    ...
    begin
      ...
      case TriaMessage(Self,'Die Klasse wurde geändert.'+#13+
...
begin
  ...
  try
    ...
    if NeuEingabe and (AkListe.Count <= 1) or
       (TriaMessage(Self,'Alle in der Liste enthaltene Klassen werden gelöscht.',
